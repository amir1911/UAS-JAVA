...
public class BusSelectionFrame extends JFrame {
    ...
    private void loadSchedules() {
        try (Connection conn = DBConfig.getConnection()) {
            ...
            String sqlBusId = "SELECT id FROM buses WHERE nama_bus = ?";
            PreparedStatement psBus = conn.prepareStatement(sqlBusId);
            psBus.setString(1, busCombo.getSelectedItem().toString());

            ResultSet rsBus = psBus.executeQuery();

            int busId = -1;
            if (rsBus.next()) busId = rsBus.getInt("id");
